...
ui <- function(config) {
  ...
  uiArgs <- list()
  # unlisting a shiny.tag fails so this is completed as a for loop
  for (module in config$shinyModules) {
    if (length(module$subModules) == 0) {
      ...
      uiArgs[[length(uiArgs) + 1]] <-
        shinydashboard::tabItem(
          tabName = module$tabName,
          eval(uiFunction)(id = module$id)
        )
    } else {
      for (subMenuItem in module$subModules) {
        uiArgs[[length(uiArgs) + 1]] <-
          shinydashboard::tabItem(
            tabName = subMenuItem$tabName,
            eval(subMenuItem$uiFunction)(id = subMenuItem$tabName,
                                         parentId = module$id)
          )
      }
    }
  ...
  return(
    shinydashboard::dashboardPage(
      ...
      shinydashboard::dashboardBody(
        do.call(shinydashboard::tabItems, uiArgs),
        shiny::tags$footer(
          shiny::h6(
            paste0(
